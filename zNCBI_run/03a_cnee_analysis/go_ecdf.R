...
for (whichont in c("BP", "MF")) {
  spec_patt <- glob2rx(paste0("GO_top1_*_", whichont, "_perm.tsv"))
...
for (whichont in c("BP", "MF")) {
  spec_patt <- glob2rx(paste0("GO_top2_*_", whichont, "_perm.tsv"))
  files <- list.files(path = wd, pattern = spec_patt, full.names = T)
  results <- list()
  for (file in files) {
    results[[file]] <- read_tsv(file) %>% 
...
for (whichont in c("BP", "MF")) {
  spec_patt <- glob2rx(paste0("GO_top3_*_", whichont, "_perm.tsv"))
  files <- list.files(path = wd, pattern = spec_patt, full.names = T)
  results <- list()
  for (file in files) {
    results[[file]] <- read_tsv(file) %>% 
