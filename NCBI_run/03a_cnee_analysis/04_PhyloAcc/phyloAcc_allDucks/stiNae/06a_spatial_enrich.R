library(tidyverse)
library(qqman)
setwd("~/Desktop/PDF/duck_assemblies/CNEEs/PhyloAcc_allDucks/stiNae")
...
bt <- function(x, n, p = 184/375591) {
  binom.test(x, n, 184/375591, alternative = "greater", conf.level = 0.95)$p.value
}
...
test <- left_join(acc.cnees.window, adjP, by = c("chr" = "chr", "start" = "start", "end" = "end")) %>%
  na.omit()
write_delim(test, "~/Desktop/PDF/duck_assemblies/CNEEs/PhyloAcc_allDucks/stiNae/test.txt", delim = "\t", col_names = F)
...
testrep <- read_delim("~/Desktop/PDF/duck_assemblies/CNEEs/PhyloAcc_allDucks/stiNae/test.rep.txt", delim = "\t", col_names = F, col_types = "dddcddddd") %>%
