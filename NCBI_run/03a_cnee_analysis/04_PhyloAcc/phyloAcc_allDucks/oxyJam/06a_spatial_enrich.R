library(tidyverse)
library(qqman)
setwd("~/Desktop/PDF/duck_assemblies/CNEEs/PhyloAcc_allDucks/oxyJam")
...
bt <- function(x, n, p = 186/375591) {
  binom.test(x, n, 186/375591, alternative = "greater", conf.level = 0.95)$p.value
}

# add binomial p-values to table
final.data.window$pVal <- mapply(bt, x = final.data.window$acc.cnee, n = final.data.window$total.cnee)
