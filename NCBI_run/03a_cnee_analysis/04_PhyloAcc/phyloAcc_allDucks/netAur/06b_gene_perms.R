...
data <- read_delim("~/Desktop/PDF/duck_assemblies/CNEEs/PhyloAcc_allDucks/netAur/cnee_perms.counts.bed", delim = "\t", col_names = F)
...
adjP %>% filter(adjP <= 0.05) %>% count() # 162 genes
...
geneList <- adjP %>% filter(adjP <= 0.05) %>% select(gene) %>% write_delim(., "~/Desktop/PDF/duck_assemblies/CNEEs/PhyloAcc_allDucks/netAur/geneList.txt", delim = "\t", col_names = T)
