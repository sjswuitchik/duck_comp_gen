...
# 300 CNEEs that are accel in all three tops
write_csv(accelerated, "acc_cnees.csv")

bed <- accelerated %>%
  mutate(newstart = start - 1) %>%
  select(chr, newstart, end, cnee) %>% 
  rename('#chrom' = chr, chromStart = newstart, chromEnd = end)
...
excl <- intersect(acc, cnee.excl.alltop) # 4 total
...
excl.cnee <- subset(bed, cnee %in% excl.vec)
final.cnee <- anti_join(bed, excl.cnee, by = "cnee")
