...
bg <- read_delim("~/Desktop/PDF/duck_assemblies/CNEEs/PhyloAcc_allDucks/oxyJam/cnee_goperms.counts.bed", delim = "\t", col_names = F) %>%
...
write_delim(bp.perms.clean, "~/Desktop/PDF/duck_assemblies/CNEEs/PhyloAcc_allDucks/oxyJam/bp.perms.clean.tsv", delim = "\t", col_names = T)
write_delim(mf.perms.clean, "~/Desktop/PDF/duck_assemblies/CNEEs/PhyloAcc_allDucks/oxyJam/mf.perms.clean.tsv", delim = "\t", col_names = T)
write_delim(cc.perms.clean, "~/Desktop/PDF/duck_assemblies/CNEEs/PhyloAcc_allDucks/oxyJam/cc.perms.clean.tsv", delim = "\t", col_names = T)

# read in permutation results if working in a new session
bp.perms.clean <- read_tsv("~/Desktop/PDF/duck_assemblies/CNEEs/PhyloAcc_allDucks/oxyJam/bp.perms.clean.tsv", col_names = T)
mf.perms.clean <- read_tsv("~/Desktop/PDF/duck_assemblies/CNEEs/PhyloAcc_allDucks/oxyJam/mf.perms.clean.tsv", col_names = T)
cc.perms.clean <- read_tsv("~/Desktop/PDF/duck_assemblies/CNEEs/PhyloAcc_allDucks/oxyJam/cc.perms.clean.tsv", col_names = T)
...
# write out GO terms
write_delim(bp.pval, "~/Desktop/PDF/duck_assemblies/CNEEs/PhyloAcc_allDucks/oxyJam/GOterms_BP.tsv", delim = "\t", col_names = T)
write_delim(mf.pval, "~/Desktop/PDF/duck_assemblies/CNEEs/PhyloAcc_allDucks/oxyJam/GOterms_MF.tsv", delim = "\t", col_names = T)
write_delim(cc.pval, "~/Desktop/PDF/duck_assemblies/CNEEs/PhyloAcc_allDucks/oxyJam/GOterms_CC.tsv", delim = "\t", col_names = T)

# write out sig GO terms
filter(bp.pval, pVal_enrich <= 0.05) %>% dplyr::select(-c(pVal_target)) %>% mutate(subontology = "BP") %>% write_delim(., "~/Desktop/PDF/duck_assemblies/CNEEs/PhyloAcc_allDucks/oxyJam/sigGOterms_BP.tsv", delim = "\t", col_names = T)
filter(mf.pval, pVal_enrich <= 0.05) %>% dplyr::select(-c(pVal_target)) %>% mutate(subontology = "MF") %>% write_delim(., "~/Desktop/PDF/duck_assemblies/CNEEs/PhyloAcc_allDucks/oxyJam/sigGOterms_MF.tsv", delim = "\t", col_names = T)
filter(cc.pval, pVal_enrich <= 0.05) %>% dplyr::select(-c(pVal_target)) %>% mutate(subontology = "CC") %>% write_delim(., "~/Desktop/PDF/duck_assemblies/CNEEs/PhyloAcc_allDucks/oxyJam/sigGOterms_CC.tsv", delim = "\t", col_names = T)
...
goList <- read_delim("~/Desktop/PDF/duck_assemblies/CNEEs/PhyloAcc_allDucks/oxyJam/sigGOterms_combined.txt", delim = "\t", col_names = T) %>% 
  dplyr::select(ID) %>%
  rename(goID = ID)
...
left_join(goList, clean.mart, by = "goID") %>% 
  na.omit() %>% 
  write_delim(., "~/Desktop/PDF/duck_assemblies/CNEEs/PhyloAcc_allDucks/oxyJam/martList.tsv", delim = "\t", col_names = T)
