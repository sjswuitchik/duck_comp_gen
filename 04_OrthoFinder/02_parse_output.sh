...
do
  ...
  sed -i 's/\"\"\"//g' compAug_data/${spp}.id | sort | uniq > compAug_data/${spp}.clean.id
  bedtools getfasta -fi compAug_data/${spp}.ncbi.fasta -bed compAug_data/${spp}.cds.gff > ${spp}.cds.fa
done
