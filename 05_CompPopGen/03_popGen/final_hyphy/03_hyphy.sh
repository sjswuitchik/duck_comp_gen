...
do
  ...
  echo -e "hyphy busted --alignment clean_align/${file}_uniq.nh" >> job_scripts_busted/run_${file}.sh
done < "clean_ogs.tsv"
...
do
  ...
  echo -e "hyphy absrel --alignment clean_align/${file}_uniq.nh" >> job_scripts_absrel/run_${file}.sh
done < "clean_ogs.tsv"
